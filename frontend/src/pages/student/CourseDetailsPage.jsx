...
export default function CourseDetailsPage() {
   ...
   const {backendURL, currency, userData, calculateChapterTime, calculateCourseDuration,
      calculateCourseRating, calculateNumberOfLectures } = useContext(AppContext);

   /**
    * fetchCourseData - fetches the course data from the backend
   ...
   /**
    * enrollInCourse - enrolls the user in the course
   ...
   async function enrollInCourse() {
      try {
         ...
         if (!userData) {
            return toast.warn('Sign in to enroll!');
         }

         if (isAlreadyEnrolled) {
            return toast.warn('User is already enrolled!');
         }

         const token = await getToken();

         const { data } = await axios.post(backendURL + '/api/user/purchase-course',
      ...
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 pt-10 text-left'>
         ...
         <div>
            ...
            <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
               <p>{calculateCourseRating(courseData)}</p>
               <div className='flex'>
                  {[...Array(5)].map((_, i) => (<img key={i} src={i < Math.floor(calculateCourseRating(courseData)) ? assets.star : assets.star_blank} alt='Full star or empty star'
         ...
         <div className="max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
            ...
            <div className="p-5">
               ...
               <button onClick={enrollInCourse} className="md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 text-white font-medium">
