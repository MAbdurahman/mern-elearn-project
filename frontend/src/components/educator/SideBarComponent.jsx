...
export default function SideBarComponent() {
   ...
   const menuItems = [
      ...
      { name: 'My Courses', path: '/educator/courses', icon: assets.my_course_icon },
      { name: 'Students Enrolled', path: '/educator/students-enrolled', icon: assets.person_tick_icon },
   ];

   return isEducator && (
      <div className='max-xs:min-w-[48px] max-xs:max-w-[48px] md:w-64 w-24 border-r min-h-screen text-base border-gray-500 py-2 flex flex-col'>
         {menuItems.map((item) => (
            <NavLink
               to={item.path}
               key={item.name}
